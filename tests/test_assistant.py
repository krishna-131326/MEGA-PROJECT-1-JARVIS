...
class MockWeatherPlugin(Plugin):
    ...
    def can_handle(self, query): return False
...
async def test_assistant_tool_loop():
    ...
    llm = FakeLLM([
        ...
        {"tool_calls": [{"function": {"name": "weather", "arguments": json.dumps({"location": "Delhi"})}}]},
    ...
    assert any(m.role == "tool" and m.tool == "weather" and "sunny in Delhi" in m.content for m in messages)
