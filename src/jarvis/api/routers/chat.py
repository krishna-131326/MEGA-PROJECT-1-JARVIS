...
async def chat(request: ChatRequest, assistant: AssistantService = Depends(get_assistant)):
...
@router.post("/stream")
async def chat_stream(request: ChatRequest, assistant: AssistantService = Depends(get_assistant)):
