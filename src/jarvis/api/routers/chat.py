...
async def chat(request: ChatRequest, assistant: AssistantService = Depends(get_assistant)) -> ChatResponse:
...
@router.post("/stream")
async def chat_stream(request: ChatRequest, assistant: AssistantService = Depends(get_assistant)) -> dict[str, str]:
