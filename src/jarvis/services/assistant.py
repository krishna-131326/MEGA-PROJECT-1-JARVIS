    ...
    async def process(self, query: str, session_id: str = "default") -> AssistantResponse:
        ...
        if plugin:
            try:
                ...
                self.memory.add_message(session_id, Message(role="assistant", content=response_text))
                return AssistantResponse(response=response_text, source="plugin", plugin_used=plugin.name)
